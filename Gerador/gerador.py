...
class Gerador:
    ...
    def geraInst(self, w):
        ...
        for i in range(self.nProj):
            p = self.geraProjeto(i, letapas, lareas, lqrn)
            P.append(p)
    ...
    def writeProj(self,p):
        ...
        texto.append('\t\t\t<Mx>' + str(p.lpar[0]) + '</Mx>\n')
        texto.append('\t\t\t<mn>' + str(p.lpar[1]) + '</mn>\n')
        texto.append('\t\t\t<a>' + str(p.lpar[2]) + '</a>\n')
        texto.append('\t\t\t<pk>' + str(p.lpar[3]) + '</pk>\n')
        texto.append('\t\t\t<mu>' + str(p.lpar[4]) + '</mu>\n')
        texto.append('\t\t\t<desvp>' + str(p.lpar[5]) + '</desvp>\n')
